...
!F1::
suspend, permit
	run, SwitchWedgeHelp06162026.html
return
...
If (OpMode == "Scan Mouse 2")
 ...
 { 
   ...
   helpTitle = Spot Scan
   helpText = Spot scan allows you to define up to 10 hot spots that will be scanned in numerical order. Identify the first hotspot by hovering the mouse cursor over the first target and capturing the mouse position with the Alt-1 key combo. The second hotspot is recorded with the Alt-2 combo, and so forth up to the tenth hotspot that is recorded with the Alt-0 key combo. Use Alt-z to clear the scan sequence. Key Trigger defines which key is used to start or stop scanning which occurs at the Scan Rate. Spot Scan 2 provides for auditory cues with the help of a clipboard text-to-speech utility like Orato or ClipSpeak. Spot Scan 1 does not offer auditory cues, but the application being scanned may be able to supply a cue as a result of a 'mouse-over' event. ----Press Enter to close this box.
